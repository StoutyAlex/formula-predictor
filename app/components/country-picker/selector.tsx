...
export function CountrySelector({ id, open, disabled = false, onToggle, onChange, selectedValue }: CountrySelectorProps) {
      ...
      <div className="mt-1 relative">
        ...
        {open && (
          ...
          >
            ...
            >
              {COUNTRIES.filter((country) => country.title.toLowerCase().startsWith(query.toLowerCase())).length === 0 ? (
                <li className="text-gray-900 cursor-default select-none relative py-2 pl-3 pr-9">No countries found</li>
              ) : (
                COUNTRIES.filter((country) => country.title.toLowerCase().startsWith(query.toLowerCase())).map((value, index) => {
                  return (
                    <li
                      key={`${id}-${index}`}
